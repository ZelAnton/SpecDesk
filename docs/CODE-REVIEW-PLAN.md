...
- **T-09** `AGENTS.md`/`CLAUDE.md` are gitignored, yet ~10 tracked/published docs link to `../AGENTS.md` as the authoritative conventions reference — every such link 404s on GitHub and the "authoritative" conventions are invisible to any clone (incl. CI agents told to consult them). **Direction:** track `AGENTS.md` (it reads as a public conventions doc) or repoint links at `CONTRIBUTING.md`/`docs/`. **Effort:** S. *(Doc change — outside the code scope, but flagged by the structure pass.)*
