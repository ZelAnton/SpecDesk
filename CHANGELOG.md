...
- While you are editing a specification, SpecDesk now shows the **other open reviews that change the same file** — the actionable version of "someone else is editing this" — and lets you compare any of them with what you have, without leaving the app. Pick a review and choose what to compare its version against: **your working copy** (exactly what you are looking at now, including unsaved edits) or the **published version**, and see the result either **formatted** (the document as it reads, with the changed parts highlighted) or as the plain **source** — the same reading/raw toggle as everywhere else. It is read-only: you see the overlapping work and understand it before you collide, without any merging, pulling, or git terminology, and the affordance simply stays hidden when the document is not in a connected GitHub repository.
- When someone else changes the same document while you are sending it for review or sharing new versions, SpecDesk now catches the overlap before it becomes a problem and walks you through it in plain language — a **"Someone else changed this too"** prompt with four clear choices instead of any technical conflict output: **Keep mine** (keep your version), **Keep theirs** (take their published version), **Combine** (keep yours and see both side by side in the normal editor to merge by hand), or **Ask for help** (leave everything exactly as it was). You never see conflict markers or git terminology, no choice can silently discard your or their work, and after you decide, the reconciled document reloads ready to send or update again.
- SpecDesk now keeps your registered local copies up to date on its own. While the window is in use it quietly checks each copy against GitHub in the background, refreshes the "updates available" and "conflict" indicators without a manual Refresh, and brings a clean main copy up to date automatically — never touching a copy that has a draft in progress or unfinished edits. It checks more often while the window is focused and eases off in the background, stays silent when there is no connection, and stops as soon as you disconnect your GitHub account or close the workspace.
- Authors can now **publish** an approved document directly from the editor: a Publish action appears once the review is approved, and it merges the open change request, removes the draft's working line, and moves the document to Published. It is available only where the repository's maintainer has enabled it (`[review] allow-author-publish` in `.spectool.toml`) and the same permission is re-checked before anything is merged; the merge is pinned to the exact reviewed version, so a change made after approval is refused with a plain reason rather than published unseen, and any failure leaves the document approved so it can be retried.
...
- The assistant can now propose an edit to the document you are drafting, and it never changes the document on its own: a confirmation appears showing exactly what would change (highlighting the difference from the current text), and you can edit the suggestion, apply it, or discard it. Only what you apply is written — through the same path as a manual edit — and if the document changed while you were reviewing the suggestion it is not applied over your work, with a plain note to ask again.
- A new **Search** mode on the left rail searches every Markdown file in the open workspace (not just the currently open document), showing each match's file, line, and a short snippet; clicking a result opens that document and scrolls straight to the match. The search stays fast and never reads outside the open workspace even on a very large tree, noting when it stopped early on an unusually large result set. The title-bar search box is unchanged and still searches only the open document.
...
- You can now create a brand-new specification without leaving SpecDesk: a **New specification** action on the Start screen and on every folder in the Disk navigator opens a small inline prompt for the name, then makes the file (a tidy, filesystem-safe name derived from what you typed) with your name as its heading and opens it ready to edit. The new file appears in the navigator right away without reloading the whole tree, new files are always created inside the folder you are working in, and a name that clashes with an existing file is refused with a plain message instead of overwriting anything.
